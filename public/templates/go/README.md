...
func main() {
    resp, err := http.Get("https://real-time-backend-preview.vercel.app/users")
    if err != nil {
        panic(err)
    }
...
```go
req, _ := http.NewRequest("GET", "https://real-time-backend-preview.vercel.app/users", nil)
...
req, _ := http.NewRequest("POST", "https://real-time-backend-preview.vercel.app/users", bytes.NewBuffer(jsonData))
req.Header.Set("Authorization", "Bearer your-token-here")
req.Header.Set("Content-Type", "application/json")
```
