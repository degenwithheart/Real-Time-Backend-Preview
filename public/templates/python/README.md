...
response = requests.get('https://real-time-backend-preview.vercel.app/users')
users = response.json()
print(users)
```
...
response = requests.get('https://real-time-backend-preview.vercel.app/users', headers=headers)
```
...
response = requests.post(
    'https://real-time-backend-preview.vercel.app/users',
    headers=headers,
    json=user_data
)
```
...
def fetch_all_users():
    ...
    while True:
        response = requests.get(f'https://real-time-backend-preview.vercel.app/users?page={page}')
        data = response.json()
